...
// ---------------------------------------------------------------------------------
//  FILENAME:      invite-user.php
//
//  DESCRIPTION:   USer Invite System 
//
...
if($numrows == 1)
{
	...
	$query = "INSERT INTO users (email, domain, type, validationGUID) ";
	$query .= "VALUES ('$invitee', '$domain', '12', '$guid');";
	$result = @mysqli_query($GLOBALS["___mysqli_ston"], $query); 
	...
	if($numrows == 1) {
	    ...
	    // --------------------------------------------------------------------------  
	    $strSubject = "You've been invited to join $domain.project-oslo.com";
	    ...
	    $strEmailBodyTemp = file_get_contents('../assets/email_creatives/user_invite.html');
		
	    // --------------------------------------------------------------------------  
	    // Now perform variable substitution on the email body.. so we are left with
	    // email body with data in it.
	    // --------------------------------------------------------------------------  
	    $original = array("%%inviter%%", "%%domain%%", "%%guid%%");
	    $replace  = array($inviter, $domain, $guid);
